"use client"
import React, {useEffect, useState} from 'react';
import Image from "next/image";
import { useRouter,useSearchParams, useParams  } from 'next/navigation'
import Modal from "../../Component/Modal"
import Head from "next/head";


const Search = () => {
    const router = useSearchParams();
    console.log(router.get('find'));


    const findStr = router.get('find')
    const param = useParams();
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedCity, setSelectedCity] = useState('Izhevsk');
    const [selectedCategory, setSelectedCategory] = useState('all');


    const [isModalOpen, setIsModalOpen] = useState(false);

    const handleOpenModal = (item) => {
        setItemModal(item)
        setIsModalOpen(true);
    }

    const handleCloseModal = () => {
        setIsModalOpen(false);
    }

    useEffect( () => {
        if (exCategory)
        {
            setSelectedCategory(exCategory);
        }
        if (exCity)
        {
            setSelectedCity(exCity)
        }

        if (findStr)
        {
            setSearchTerm(findStr)
        }
    },[])


    let exCategory = '';
    let exCity ='';

    if (param.slug == 1)
    {
        exCategory = 'Кислородные концентраторы';
        console.log('KK')
    }

    if (param.slug == 2){
        exCategory = 'Средства реабилитации';
        console.log('SR')
    }

    if (param.slug == 3){
        exCategory = 'Приборы';
        console.log('P')
    }

    if (param.slug == 4)
    {
        exCategory = 'Комплектующие';
        console.log('K')
    }
    if (param.slug == 5)
    {
        exCategory = 'Кровати';
        console.log('K')
    }

    if (param.slug == 'Izhevsk')
    {
        exCity = 'Izhevsk';
    }
    if (param.slug == 'Ekaterinburg')
    {
        exCity = 'Ekat';
    }

    if (param.slug == 'Krasnodar')
    {
        exCity = 'Krasnodar';
    }





    const data = {
        items:[
            {
                id: 1,
                title: 'Кислородный концентратор 3 Л.',
                img: 'VentumSmart3.jpg',
                desc: 'Производительность: 3л/мин.' +
                    '(Ventum Smart 3 и его аналоги)',
                longDesc:'Рото-носовые СиПАП-маски закрывают область рта и носа, и рекомендуются людям, которые спят с открытым ртом. Мягкий силикон маски BMC не натирает кожу, фиксация с помощью эластичных ремней на голове гарантирует отсутствие протечек и декомпрессии. Специальный клапан, встроенный в угловой коннектор, обеспечивает безопасность дыхания пациента и защиту от удушья в случае экстренного отключения лечебного аппарата или блокировки воздушного потока. Специальные фиксаторы-липучки на ремнях крепления позволяют надежно закрепить рото-носовую маску BMC на голове. Настроив ремни крепления один раз, последующие снятия маски осуществляются с помощью специальных клипс (защелок) на передней части маски. Угловой коннектор, к которому присоединяется дыхательная трубка, свободно вращается на 360 это позволяет избежать скручивания трубки во время сна. Рото-носовая маска BMC подходит для всех СиПАП и БиПАП-аппаратов со стандартным креплением диаметром 22мм и используется взрослыми пациентами с массой тела более 30 кг. В комплекте с шапочкой.',
                category: 'Кислородные концентраторы',
                priceA: '1 500 ₽. неделя',
                price: '',
                city:'Izhevsk'
            },
            {
                id: 2,
                title: 'Кислородный концентратор 10 Л.',
                img: 'armedJ10.jpg',
                desc: 'Производительность: 10 л/мин. ' +
                    '(Armed JAY 10 и его аналоги)',
                category: 'Кислородные концентраторы',
                priceA: '3 000 ₽. неделя',
                price: '',
                city:'Izhevsk'
            },
            {
                id: 3,
                title: 'Кислородный концентратор 10 Л.',
                img: 'Armed10L.jpg',
                desc: 'Производительность: 10л/мин.' +
                    '(Armed 10-L и его аналоги)',
                category: 'Кислородные концентраторы',
                priceA: '4 000 ₽ неделя',
                price: '',
                city:'Izhevsk'
            },

            {
                id: 5,
                title: 'Костыли подлокотные (канатка)',
                img: 'Kostil.jpg',
                desc: 'C подлокотником и устройством против скольжения',
                longDesc: 'Костыли с опорой под локоть предназначены для дополнительной опоры и облегчения передвижения людей имеющих травмы и заболевания нижних конечностей.\n'+
                    'Вес костыля: 0.58 кг.\n'+
                    'Выдерживают нагрузку до 120 кг.\n'+
                    'Диапазон регулировки высоты костыля:\n'+
                    'Высота костыля от пола до ручки: Мин-765 mm, Макс-990 mm.\n'+
                    'Размер от ручки до подлокотника: 230 mm.\n',
                category: 'Средства реабилитации',
                priceA: '490 ₽. месяц(за пару)',
                price: '2490',
                city:'Izhevsk'
            },
            {
                id: 6,
                title: 'Ходунки XS305',
                img: 'Hodunki.jpg',
                desc: 'Средство реабилитации',
                longDesc: 'Ходунки Ortonica XS305 предназначены для людей с нарушением опорно-двигательного аппарата.' +
                    'Замок на передней части конструкции позволяет передвигаться как в фиксированном, так и в шагающем режиме.' +
                    'Конструкция ходунков складывается и раскладывается за счет нажатия на фиксаторы пальцами, ладонью или ребром кисти.' +
                    'Ножки подстраиваются под рост пользователя легким нажатием на кнопочный замок, уменьшая нагрузку на суставы.\n' +
                    'Технические характеристики\n' +
                    'Тип - Ходунки\n' +
                    'Цвет -  Серебристый матовый \n' +
                    'Длина - в сложенном состоянии, см - 10 \n' +
                    'Общая высота, см - 75,5-95,5\n' +
                    'Модель - XS305\n' +
                    'Материал - Алюминий\n' +
                    'Вес, кг (± 5%) 2,45\n' +
                    'Общая ширина, см - 57-59\n' +
                    'Расстояние между поручнями, см - 46\n' +
                    'Грузоподъемность, кг 100\n' +
                    'Общая длина, см 45-49\n',
                category: 'Средства реабилитации',
                price: '2 990',
                priceA: '490 ₽. месяц',
                city:'Izhevsk'
            },
            {
                id: 7,
                title: 'Трости',
                img: 'Trost.jpg',
                desc: 'Средство реабилитации',
                category: 'Средства реабилитации',
                priceA: 'от 250 за месяц ',
                price: 'от 790',
                city:'Izhevsk'
            },
            {
                id: 10,
                title: 'ОФТН-03',
                img: 'ОФТН-03.jpg',
                desc: 'Фотолампа для лечения желтухи новорожденных',
                longDesc: 'Особенности и преимущества облучателя фототерапевтического для лечения желтухи новорожденных ОФТН-03-"Аксион":\n' +
                    'Комфортные условия проведения сеанса фототерапии с большой эффективностью;\n' +
                    'Регулировка интенсивности излучения для оптимального режима лечения;\n' +
                    'Возможность работы излучателей до 50 000 часов за счет использования сверхярких светодиодов в качестве источников излучения;\n' +
                    'Индикация заданного времени сеанса облучения;\n' +
                    'Индикация текущего времени сеанса облучения с дискретностью 1 минута, с точностью отсчета не более 1%;\n' +
                    'Автоматическое выключение и звуковая сигнализация окончания сеанса облучения;\n' +
                    'Индикация времени наработки./n'+
                    'Регулировка интенсивности излучения\n' +
                    'Индикация заданного времени сеанса облучения\n' +
                    'Автоматическое выключение и звуковая сигнализация окончания сеанса\n' +
                    'Индикация времени наработки',
                category: 'Приборы',
                price: '',
                priceA: '590 ₽. за сутки',
                city:'Izhevsk'
            },
            {
                id: 11,
                title: 'BMC RESMART AUTO G2 ',
                img: 'SipUp.jpeg',
                desc: 'Автоматический CPAP для лечения ночного апное',
                longDesc: 'ReSmart Auto G2 – современный аппарат для СИПАП-терапии от известного немецко-китайского концерна BMC.\n' +
                    'Устройство появилось на рынке медицинского оборудования недавно и мгновенно заняло лидирующие позиции в нише бюджетных устройств,\n' +
                    'соединив в себе отличную цену и превосходное качество. ReSmart Auto G2 обладает уникальным широким функционалом,\n' +
                    'что обеспечивает высокую эффективность и комфорт СИПАП-терапии. Техника отличается привлекательным эргономичным дизайном,\n' +
                    'надежностью и простотой управления.\n' +
                    'рекомендуется известными врачами-сомнологами.',
                category: 'Приборы',
                price: '42 990',
                priceA: '7 990 ₽. за месяц',
                city:'Izhevsk'
            },
            {
                id: 12,
                title: 'iWALK3.0',
                img: 'iWalk.jpg',
                desc: 'Инновационный подколенный костыль - свободные руки',
                longDesc: 'Инновационный костыль iWALK отмечен наградами. iWALK - лучшая альтернатива обычным костылям и коленным самокатам при всех травмах ниже колена.\n'+
                    'Костыль iWALK дает вам мобильность без помощи рук / боли и возвращает вам возможность вести функциональный и независимый образ жизни.\n'+
                    'С костылем iWALK снова можно толкать тележку для покупок, подниматься по лестнице, гулять с собакой и ходить на работу.',
                category: 'Средства реабилитации',
                price: '',
                priceA: '2 990 ₽. за месяц',
                city:'Izhevsk'
            },
            {
                id: 13,
                title: 'Весы детские Саша',
                img: 'VesSasha.jpg',
                desc: 'Инновационный подколенный костыль - свободные руки',
                longDesc: 'Весы используются для взвешивания новорожденных и грудных детей как в детских медицинских учреждениях,' +
                    ' так и в домашних условиях. Весы зарегистрированы как медицинское изделие и имеют регистрационное удостоверение \n' +
                    'Федеральной службы по надзору в сфере здравоохранения № РЗН 2013/1197 от 09.04.2018 г. Весы просты в эксплуатации, \n' +
                    'они помогут осуществлять постоянный контроль за прибавкой веса ребенка в первые годы его жизни. \n' +
                    'Яркая подсветка ЖК-дисплея позволяет производить взвешивание во время ночных кормлений младенца. \n' +
                    'Встроенный аккумулятор обеспечивает автономную работу весов до 94 часов. \n' +
                    'Допускается санобработка весов (дезинфекция и облучение кварцем).\n',
                category: 'Приборы',
                price: '',
                priceA: '1 090 ₽. за месяц',
                city:'Izhevsk'
            },
            {
                id: 14,
                title: 'Otto Bock "Старт"',
                img: 'InvalidCalManual.jpg',
                desc: 'Инвалидная коляска с ручным приводом',
                longDesc: 'Инвалидная коляска" СТАРТ" предназначена для людей с разной степенью активности. \n' +
                    'Адаптер приводного колеса позволяет отрегулировать маневренность коляски под нужды каждого конкретного пользователя.\n'+

                    'Инвалидная коляска" Старт" оснащена пневматическими приводными и поворотными колесами, '+
                    'что обеспечивает комфортную езду по любым покрытиям и существенно упрощает техническое обслуживание.\n'+

                    'Кресло коляска оборудована откидывающимися боковинами с регулируемыми по высоте подлокотниками.\n'+
                    'Подножки регулируются по высоте, откидываются в стороны или снимаются совсем. Складная конструкция\n' +
                    ' рамы обеспечивает компактное хранение инвалидной коляски в домашних условиях и транспортировку\n' +
                    'в багажнике автомобиля.\n'+
                    'Технические характеристики:\n' +
                    '- Макс. нагрузка 125 кг.;\n' +
                    '- Ширина сиденья 38-50,5 см.;\n' +
                    '- Полная ширина 59,5-74,5 см.;\n' +
                    '- Полная длина с подножкой 70-110 см.;\n' +
                    '- Ширина сложенного кресла-коляски 20-40 см.;\n' +
                    '- Высота сиденья спереди 42-51,5 см.;\n' +
                    '- Высота спинки 42-47,5 см.;\n' +
                    '- Угол наклона спинки -30-90°;\n' +
                    '- Угол наклона сиденья -5-15°;\n' +
                    '- Длина подножки 36-47 см.;\n' +
                    '- Угол наклона подножки 75-90°;\n' +
                    '- Расстояние от подлокотника до обивки 18-36 см.;\n' +
                    '- Диаметр ручного обода 50-60 см.\n' +
                    'Спинка с анатомическим люмбальным изгибом\n' +
                    'Черное сидение и спинка Сиденье шириной 45.5 см изготовленo из нейлона, обладающего водоотталкивающими свойствами\n' +
                    'Конструкция кресла-коляски предоставляет возможности регулировки:\n' +
                    '- Высоты спинки и глубины сидения;\n' +
                    '- Высоты сиденья;\n' +
                    '- Положения подножек, которые можно откинуть вверх, вбок или совсем снять;\n' +
                    '- Высоты подлокотников, расположенных на боковинах.',
                category: 'Приборы',
                price: '12 990',
                priceA: '1 980 ₽. за месяц',
                city:'Izhevsk'
            },
            {
                id: 15,
                title: 'СиПАП-маска',
                img: 'RotoNos.jpg',
                desc: 'Ротоносовая СиПАП-маска',
                longDesc: 'Рото-носовые СиПАП-маски закрывают область рта и носа,' +
                    'и рекомендуются людям, которые спят с открытым ртом.' +
                    'Мягкий силикон маски BMC не натирает кожу, фиксация с помощью эластичных ремней' +
                    'на голове гарантирует отсутствие протечек и декомпрессии.' +
                    'Специальный клапан, встроенный в угловой коннектор,' +
                    'обеспечивает безопасность дыхания пациента и защиту от удушья в случае экстренного отключения' +
                    ' лечебного аппарата или блокировки воздушного потока. Специальные фиксаторы-липучки' +
                    ' на ремнях крепления позволяют надежно закрепить рото-носовую маску BMC на голове. ' +
                    'Настроив ремни крепления один раз,' +
                    'последующие снятия маски осуществляются с помощью специальных клипс (защелок)' +
                    'на передней части маски. Угловой коннектор, к которому присоединяется' +
                    'дыхательная трубка,\n' +
                    'свободно вращается на 360 это позволяет избежать скручивания трубки во время сна.' +
                    'Рото-носовая маска BMC подходит для всех СиПАП и БиПАП-аппаратов со стандартным креплением' +
                    'диаметром 22мм и используется взрослыми пациентами с массой тела более 30 кг.' +
                    'В комплекте с шапочкой.',
                category: 'Комплектующие',
                price: '5 500',
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 16,
                title: 'Ходунки роллаторы',
                img: 'HodunkiRollator.jpg',
                desc: 'Ротоносовая СиПАП-маска',
                longDesc: 'ходунки-роллаторы с колесами, обеспечивающими удобную и легкую эксплуатацию.\n' +
                    'Сиденье позволяет использовать устройство в качестве стула, а вместительная корзина поможет\n' +
                    'с хранением личных вещей во время прогулки.\n'+
                    'Ручки управления оснащены специальными ручными тормозами и удобными накладками, которые предотвращают соскальзывание ладоней.\n'+
                    'Разборная конструкция ходунков позволяет оперативно сложить изделие, что особенно полезно при транспортировке и хранении.\n'+
                    'Важно! В ассортименте несколько видов ходунков - на колесах, ходунки-роллаторы, ходунки с сиденьем.\n'+
                    'Возможна срочная доставка\n'+
                    'Большой выбор средств реабилитации у нас на сайте!\n'+
                    'Основные характеристики:\n'+
                    'Максимальная нагрузка: 110 кг\n'+
                    'Высота: 80-92 см\n'+
                    'Длина: 76 см\n'+
                    'Ширина: 58,5 см'+
                    'Ширина между поручнями: 43 см\n',
                category: 'Средства реабилитации',
                price: '5 990',
                priceA: '890 за месяц',
                city:'Izhevsk'
            },
            {
                id: 17,
                title: 'Депортационный ортез',
                img: 'DeportArtez.jpg',
                desc: 'Для голеностопного сустава',
                longDesc: 'Предназначен для фиксации положения нижней конечности с возможностью коррекции степени внутренней и наружной ротации\n'+
                    'Показания к применению:\n'+
                    'Период реабилитации после гипсовых повязок, эндопротезирования тазобедренного сустава\n'+
                    'Переломы шейки и вертельной области бедра\n'+
                    'Невропатия, парезы и параличи нижних конечностей\n'+
                    'Период после инсультов и очаговых поражений нервов нижних конечностей\n'+
                    'В качестве лонгеты на ночь при отвисающей стопе\n'+
                    'Особенности конструкции:\n'+
                    'Подходит для правой и левой ноги\n'+
                    'В комплекте есть клиновидный пелот для изменения угла сгибания стопы\n'+
                    'Надевается как на тело, так и на тонкий хлопчатобумажный трикотаж\n'+
                    'Материал: пластик с мягкой внутренней вставкой. Состав: 60% пластмасса, 40% полиэстер\n',
                category: 'Средства реабилитации',
                price: '4 990',
                priceA: '1 490 р за месяц',
                city:'Izhevsk'
            },
            {
                id: 18,
                title: 'Канюля назальная 1.8м',
                img: 'KanulaNOS.jpg',
                desc: 'Для кислородного концентра',
                longDesc: 'Предназначена для проведения кислородной терапии с кислородным концентратором.' +

                    'Канюля назальная – это кислородный шланг, который необходим для проведения кислородной терапии' +
                    'с использованием кислородного концентратора. Один конец канюли присоединяется к аппарату,' +
                    'который выделяет из воздуха кислород, другой – с двумя наконечниками – вставляется в носовые проходы пациента.' +
                    'Таким образом, процедура становится гораздо удобнее:' +
                    'пациент может спокойно разговаривать или есть, да и руки остаются свободными.' +
                    'Назальная канюля имеет в длину 1.8 метра и предназначена для взрослых.' +
                    'Она изготовлена из мягких полимерных материалов,' +
                    'поэтому комфортная на ощупь, легкая и при этом прочная.',
                category: 'Комплектующие',
                price: '390',
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 19,
                title: 'Oxyco 16 л.',
                img: 'Oxyco.jpg',
                desc: 'Кислородный баллончик',
                longDesc: 'Кислородный баллончик OXYCO 16 л. (10 штук + мягкая маска)\n'+
                    '- прекрасная альтернатива кислородной подушки\n'+
                    '- временная замена кислородного концентратора (например, по дороге ко врачу)\n'+
                    '- источник кислорода для приготовления кислородных коктейлей\n'+
                    'Кислородного баллончика объемом 16 литров хватает до 140 вдохов или на приготовление не менее 30 порций кислородных коктейлей.\n',
                category: 'Комплектующие',
                price: '10 шт. за 5 200',
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 20,
                title: 'Маска кислородная с мешком',
                img: '1MaskaMeshok.jpg',
                desc: 'Кислородный баллончик',
                longDesc: 'Маска кислородная с мешком и соединительной трубкой 2м \n' +
                    '\n' +
                    'Кислородная маска с мешком и соединительной трубкой 2 метра размер XL\n' +
                    '\n' +
                    'Алюминиевый носовой фиксатор.Эластичный фиксирующий ремешок. \n' +
                    'Переходник на маске для кислородной трубки с двумя дополнительными компенсаторными отверстиями. \n' +
                    'Два боковых клапана на теле маски. Не содержит латекса. ' +
                    'Наличие контрольного клапана в переходнике для кислородной трубки, наличие резервного дыхательного мешка. \n' +
                    'Трубка для соединения с источником кислорода, длина 2 м с универсальным коннектором\n',
                category: 'Комплектующие',
                price: '890',
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 21,
                title: 'Фильтр медицинский стерильный',
                img: '2FilterAnastezia.jpg',
                desc: 'Для анестезиологии',
                longDesc: 'Фильтры медицинские стерильные для анестезиологии и вентиляции лёгких, аэрозольной и кислородной терапии \n' +
                    'Изделие, предназначенное для помещения в дыхательный контур проксимально относительно пациента \n' +
                    'для удаления и удержания микробов при захвате образовавшегося в результате выдоха пациента тепла \n' +
                    'и влаги для их использования с целью нагрева и увлажнения терапевтических газов, вдыхаемых пациентом,\n ' +
                    'а также может использоваться для фильтрации медицинских газов. Может также фильтровать другие мелкие частицы.\n' +
                    ' Изделие содержит бактериальный фильтр, \n' +
                    'предназначенный для снижения риска перекрестного заражения между пациентами, \n' +
                    'и тепловлагообменный слой для поддержания нормального состояния слизистых оболочек дыхательных путей. \n' +
                    'Имеет порт «Луер» для подключения линии мониторинга газов (капнометрия). \n' +
                    'Это стерильное изделие одноразового использования.',
                category: 'Комплектующие',
                price: '150',
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 22,
                title: 'Фильтр бактериальный',
                img: '3PulseOksimeter.jpg',
                desc: 'Фильтр бактериальный',
                longDesc: 'Один из самых доступных по цене портативных пульсоксиметров со встроенным датчиком. \n' +
                    'Очень удобен в использовании: просто наденьте его на палец,\n' +
                    'и уже через несколько секунд на LED-дисплее высветится процентное содержание кислорода в крови\n'+
                    'Малый вес (всего 28грамм без батареи).\n'+
                    'Дисплей SpO2, ЧСС, Пульс-бар.\n'+
                    'Низкое потребление энергии, автоматическое отключение.\n'+
                    'Индикатор заряда батареи.\n'+
                    'Работает от 2-х щелочных батареек ААА (или аккумуляторов).'
                ,
                category: 'Приборы',
                price: '2490',
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 23,
                title: 'ChoiceMMed MD300C12',
                img: '4FilterAntiBackCon.jpg',
                desc: 'Пульсоксиметр',
                longDesc: 'Один из самых доступных по цене портативных пульсоксиметров со встроенным датчиком. \n' +
                    'Очень удобен в использовании: просто наденьте его на палец,\n' +
                    'и уже через несколько секунд на LED-дисплее высветится процентное содержание кислорода в крови\n'+
                    'Малый вес (всего 28грамм без батареи).\n'+
                    'Дисплей SpO2, ЧСС, Пульс-бар.\n'+
                    'Низкое потребление энергии, автоматическое отключение.\n'+
                    'Индикатор заряда батареи.\n'+
                    'Работает от 2-х щелочных батареек ААА (или аккумуляторов).'
                ,
                category: 'Приборы',
                price: '150',
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 24,
                title: 'Фетальный допплер',
                img: '5Dopper.jpg',
                desc: 'Фетальный допплер',
                longDesc: 'Этот прибор позволяет в домашних условиях услышать, как бьётся сердечко вашего малыша. ' +
                    'Устройство безопасное и простое в использовании ✅ все показания сердцебиения выводятся на экран дисплея.'+
                    'позволяет услышать сердце малыша уже с 14-16 недели.'+
                    'Приобретая допплер, вы приобретаете спокойствие на весь период вашей беременности'
                ,
                category: 'Приборы',
                price: '2 490',
                priceA: '490 ₽. месяц',
                city:'Izhevsk'
            },
            {
                id: 25,
                title: 'Алмаг плюс',
                img: '6Almag.jpg',
                desc: 'Аппарат магнитотерапевтический',
                longDesc: 'Показания к применению\n' +
                    'Артриты, артрозы, остеохондропатии, пяточная шпора\n' +
                    'Дорсопатия (остеохондроз позвоночника, грыжа межпозвоночного диска, сколиоз)\n' +
                    'Остеопороз\n' +
                    'Гипертоническая болезнь I, II степени\n' +
                    'Дистония (вегетососудистая дистония)\n' +
                    'Осложнения сахарного диабета I и II типа\n' +
                    'Атеросклероз\n' +
                    'Болезни вен и лимфатических сосудов (включая варикозную болезнь и ее осложнения, лимфостаз)\n' +
                    'Бронхиальная астма\n' +
                    'Бронхит\n' +
                    'Поражения отдельных нервных корешков и сплетений верхних и нижних конечностей, в том числе после травм, перенесенного инсульта\n' +
                    'Травмы (переломы)'
                ,
                category: 'Приборы',
                price: '',
                priceA: '750 р за 10 дней',
                city:'Izhevsk'
            },
            {
                id: 26,
                title: 'Диффузор',
                img: '7Diffuzor.jpg',
                desc: 'Для кислородного концентратора Армед',
                longDesc: 'Диффузор предназначен для проведения кислородотерапии в профилактических целях с помощью кислородного концентратора. Отличительная его особенность - наличие разборной аромакапсулы. Поместив в нее пропитанную ароматической жидкостью губку, пользователь сможет проводить процедуры кислородной ароматерапии.\n' +
                    '\n' +
                    'Основные характеристики:\n' +
                    '— Материал Пластик;\n' +
                    '— Возраст Взрослая;\n' +
                    '— Используется вместе с Концентратор кислорода;\n' +
                    '— Размер Безразмерный;\n' +
                    '— Колба для ароматерапии да;\n' +
                    '— Объем колбы для ароматерапии 20 мл;\n' +
                    '— Гибкая трубка с распылителем да.'
                ,
                category: 'Комплектующие',
                price: '490',
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 27,
                title: 'Увлажнитель (стакан)',
                img: '8Stakan.jpg',
                desc: 'для кислородных концентратора',
                longDesc: 'Увлажнитель для концентратора кислорода HUM.Универсальный пузырьковый увлажнитель для концентраторов кислорода. Подходит к большинству концентраторов кислорода. Объем увлажнителя кислорода 350 мл.\n' +
                    '\n' +
                    'Произведен в Германии. \n' +
                    '\n' +
                    'Многоразовый пузырьковый увлажнитель подходит к следующим моделям кислородных концентраторов:\n' +
                    '\n' +
                    'BITMOS OXY 6000 (6 L)\n' +
                    'NIDEK Mark 5 Nuvo Lite \n' +
                    'NIDEK Mark 5 Nuvo 8\n' +
                    'Philips Respironics EverFlo\n' +
                    'Invacare PerfectO2\n' +
                    'AirSep VisionAir\n' +
                    'AirSep Elite NewLife Single (5л)\n' +
                    'AirSep Elite NewLife Single (8л)\n' +
                    'AirSep Elite NewLife Dual (8л)\n' +
                    'AirSep Elite NewLife Single (10л)л\n' +
                    'AirSep Elite NewLife Life Dual (10л)\n' +
                    'Longfian Jay 3\n' +
                    'Longfian Jay 5A,\n' +
                    'Longfian Jay 10 (с одним выходом)\n' +
                    'Longfian Jay 10 (с двумя выходами)\n' +
                    'Canta HG-5S,\n' +
                    'Canta HG-5WN'
                ,
                category: 'Комплектующие',
                price: '999',
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 28,
                title: 'Алмаг-01',
                img: '10Alamag01.jpg',
                desc: 'Аппарат магнитотерапевтический',
                longDesc: 'Пpоkaт. Аппарат магнитотерапевтический.\n' +
                    'Стоимость проката: 10 дней - 700 р..\n' +
                    'Пpokaт. Аппарат магнитотерапевтический.\n' +
                    'Современный аппарат предназначен для лечения ряда заболеваний:\n' +
                    'Артрит\n' +
                    'Артроз\n' +
                    'Остеохондроз грудного и поясничного отделов позвоночника.\n' +
                    'Травмы опорно-двигательного аппарата.\n' +
                    'Вегето-сосудистая дистония.\n' +
                    'Подагра.\n' +
                    'Невралгия.\n' +
                    'Гастрит, панкреатит, язва.\n' +
                    'Бронхиальная астма\n' +
                    'Благодаря точным характеристикам аппарат даёт возможность:\n' +
                    'устранить боль;\n' +
                    'прекратить воспалительный процесс,\n' +
                    'остановить дегенеративные изменения;\n' +
                    'улучшить работоспособность и качество жизни.\n' +
                    'Полезные умения аппарата:\n' +
                    'снижает вязкость крови, улучшает проницаемость капилляров, нормализует обмен веществ и кровообращение;\n' +
                    'решает проблему питания и дефицита кислорода в больном органе и окружающих мышцах, снабжает ткани иммуноглобулинами и строительными белками;\n' +
                    'быстро доставляет лекарственные препараты и помогает их усвоению,\n' +
                    'создаёт условия для выздоровления либо стойкой ремиссии.\n' +
                    'Аппарат нормализует жизнедеятельность организма, восстанавливает функции органов, улучшает подвижность и жизненную активность.\n' +
                    'На ранней стадии заболевания или для профилактики обострений аппарат может успешно выступать в качестве монотерапии артрита, артроза, остеохондроза, подагры, травматических повреждений.'
                ,
                category: 'Приборы',
                price: '',
                priceA: '700 за 10 дней',
                city:'Izhevsk'
            },
            {
                id: 30,
                title: 'Канадки',
                img: 'Kanadki.jpg',
                desc: 'Костыли подлокотные взрослые',
                longDesc: 'Характеристики:\n' +
                    'Вес пары костылей: 1.08 кг.\n' +
                    'Выдерживают нагрузку до 120 кг.\n' +
                    'Высота костыля от пола до ручки: Мин-765 mm, Макс-990 mm.\n' +
                    'Размер от ручки до подлокотника: 230 mm.'
                ,
                category: 'Средства реабилитации',
                price: '2 490' ,
                priceA: '',
                city:'Izhevsk'
            },
            {
                id: 31,
                title: 'Канадки',
                img: 'Kanadki.jpg',
                desc: 'Костыли подлокотные взрослые',
                longDesc: 'Характеристики:\n' +
                    'Вес пары костылей: 1.08 кг.\n' +
                    'Выдерживают нагрузку до 120 кг.\n' +
                    'Высота костыля от пола до ручки: Мин-765 mm, Макс-990 mm.\n' +
                    'Размер от ручки до подлокотника: 230 mm.'
                ,
                category: 'Средства реабилитации',
                price: '2 790' ,
                priceA: '',
                city:'Krasnodar'
            },
            {
                id: 32,
                title: 'Канадки',
                img: 'Kanadki.jpg',
                desc: 'Костыли подлокотные взрослые',
                longDesc: 'Характеристики:\n' +
                    'Вес пары костылей: 1.08 кг.\n' +
                    'Выдерживают нагрузку до 120 кг.\n' +
                    'Высота костыля от пола до ручки: Мин-765 mm, Макс-990 mm.\n' +
                    'Размер от ручки до подлокотника: 230 mm.'
                ,
                category: 'Средства реабилитации',
                price: '2 790' ,
                priceA: '',
                city:'Ekat'
            },
            {
                id: 33,
                title: 'Тутор ортез',
                img: 'TutorOrtez.jpg',
                desc: 'Шина на коленный сустав',
                longDesc: 'Характеристики:\n' +
                    'Тип товара: Коленный сустав\n'+
                'Степень фиксации: жесткая (иммобилизация)\n'+
                'Размер- универсальный\n'+
                'Цвет -чёрный\n'
                ,
                category: 'Средства реабилитации',

                priceA: '1 490 р за месяц',
                city:'Krasnodar'
            },
            {
                id: 34,
                title: 'Ортез (Брейс)',
                img: 'OrtezBreis.jpg',
                desc: 'На коленный сустав',
                longDesc: 'Характеристики:\n' +
                    'Характеристики :\n'+
    'Ортез коленный (шарнирный) с регулировкой угла сгиба от 0-120 градусов; Подходит для роста от 150-190 см;\n'+
    '-Охват щиколотки: максимум 45 см\n'+
    '- Охват икры: максимум 55 см\n'+
    '-Охват над коленкой: максиму 60 см\n'+
    '-Охват бедра: максимум 65 см Подойдет как женщине так и мужчине.\n'
                ,
                category: 'Средства реабилитации',
                price: '5 980',
                city:'Krasnodar'
            },
            {
                id: 35,
                title: 'Сипап-аппарат вmc Auto G2S',
                img: 'SipapAutoBCM.jpg',
                desc: 'от храпа и апноэ',
                longDesc: 'BMC G2S A20 - это CPAP-аппарат с автоматической регулировкой давления и встроенным увлажнителем, предназначенный для лечения обструктивного апноэ сна (OSA) у взрослых в больнице или дома. ✅Благодаря приятному внешнему виду, стабильной работе, отличным комфортным характеристикам, вы и ваша семья сможете распрощаться с проблемой храпа и наслаждаться сном всю ночь'
                ,
                category: 'Приборы',
                price: '39 990',
                city:'Krasnodar'
            },
            {
                id: 36,
                title: 'Костыли',
                img: 'KostilArmed.jpg',
                desc: 'Для взрослых и детей',
                longDesc: 'Костыли подмышечные лёгкие и удобные с мягкими подушечками регулируются под рост Человека.\n'+
                    'Вес пары костылей: 1.07 кг.\n' +

                    'Выдерживают нагрузку до 120 кг.\n' +
                    'Материал костыля:Алюминиевый сплав\n'

                ,
                category: 'Средства реабилитации',
                price: '2 790',
                priceA: '590 р. за месяц',
                city:'Krasnodar'
            },
            {
                id: 38,
                title: 'Ходунки',
                img: 'Hodunki.jpg',
                desc: 'Для взрослых и детей',
                longDesc:
                'Общая длина - 47 см\n'+
                'Ширина в сложенном состоянии - 54,5 см\n'+
                'Общая высота - 75,5-95,5 см\n'+
                'Общая ширина - 63 см\n'+
                'Расстояние между поручнями - 44 см\n'+
                'Вес - 2,45 кг\n'+
                'Грузоподъемность - 100'

                ,
                category: 'Средства реабилитации',
                price: '3290',
                priceA: '590 р. за месяц',
                city:'Krasnodar'
            },
            {
                id: 39,
                title: 'Ortonica XR 201',
                img: 'OttornicaXR.jpg',
                desc: 'Роллатор коленный ',
                longDesc:
                    'Характеристики:\n'+
                    '• Складные\n'+
                    '• Материал сталь\n'+
                    '• Длина в сложенном виде 79 см\n'+
                    '• Общая высота 79-103 см\n'+
                    '• Общая ширина 42 см\n'+
                    '• Общая длина 79/89 см\n'+
                    '• Ширина в сложенном виде 42 см\n'+
                    '• Длина в сложенном виде 79 см\n'+
                    '• Диаметр колес 18,5 см\n'+
                    '• Грузоподъемность 110 кг\n'+
                    '• Вес 13 кг'
                ,
                category: 'Средства реабилитации',
                priceA: '3490 р. за месяц',
                city:'Krasnodar'
            },
            {
                id: 40,
                title: 'Otto Bock',
                img: 'KresloOtto.jpg',
                desc: 'Инвалидное кресло ',
                longDesc:
                    '🔥Инвалидная коляска Ottоbock УДОБНАЯ,КОМПАКТНАЯ,НАДЕЖНАЯ И БЕЗОПАСНАЯ. \n' +
                    '✅Она компактная, легко складывается. \n' +
                    '✅Маневренная, устойчива к опрокидыванию. \n' +
                    '✅Проста в использовании, хорошо моется. \n' +
                    '✅Подходит для дома и улицы. \n' +
                    '✅Проходит в узкие двери и лифты. \n' +
                    '✅Удобна для пользователя и сопровождающего.\n'+
                    'Характеристики: ▪️\n' +
                    'Ширина сидения 45,5 см \n' +
                    'Подходит людям с размером одежды 42-52 ▪️\n' +
                    'Общий габарит коляски 65 см ▪️\n' +
                    'Максимальная нагрузка 125кг ▪️\n' +
                    'Вес коляски 17.5 кг (Облегченная рама) ▪️\n' +
                    'Универсальная'
                ,
                category: 'Средства реабилитации',
                priceA: '2490 р. за месяц',
                city:'Krasnodar'
            },
            {
                id: 41,
                title: 'Подъемное приспособление в кровать PB-01',
                img: 'Krovat.jpg',
                desc: 'Кровать с электроприводом',
                longDesc:
                    'Специальное подъемное приспособление в кровать с электроприводом\n'+
                    'Позволяет по нажатию кнопки менять положение лежачего пользователя\n'+
                'Переводит пользователя из положения лежа в сидячее положение, или в положение полулежа\n'+
                'Регулировка угла наклона от 5° до 70°\n'+
                'Управление происходит с помощью специального пульта\n'+
                'Устройство работает от электросети или батареек типа Крона (6LR61)\n'+
                'Максимальный вес пользователя — 100 кг'
                ,
                category: 'Кровати',
                price: '14 990',
                city:'Izhevsk'
            },
            {
                id: 42,
                title: 'Подъемное приспособление в кровать PB-01',
                img: 'Krovat.jpg',
                desc: 'Кровать с электроприводом',
                longDesc:
                    'Специальное подъемное приспособление в кровать с электроприводом\n'+
                    'Позволяет по нажатию кнопки менять положение лежачего пользователя\n'+
                    'Переводит пользователя из положения лежа в сидячее положение, или в положение полулежа\n'+
                    'Регулировка угла наклона от 5° до 70°\n'+
                    'Управление происходит с помощью специального пульта\n'+
                    'Устройство работает от электросети или батареек типа Крона (6LR61)\n'+
                    'Максимальный вес пользователя — 100 кг'
                ,
                category: 'Кровати',
                price: '14 990',
                city:'Ekat'
            },
            {
                id: 43,
                title: 'Подъемное приспособление в кровать PB-01',
                img: 'Krovat.jpg',
                desc: 'Кровать с электроприводом',
                longDesc:
                    'Специальное подъемное приспособление в кровать с электроприводом\n'+
                    'Позволяет по нажатию кнопки менять положение лежачего пользователя\n'+
                    'Переводит пользователя из положения лежа в сидячее положение, или в положение полулежа\n'+
                    'Регулировка угла наклона от 5° до 70°\n'+
                    'Управление происходит с помощью специального пульта\n'+
                    'Устройство работает от электросети или батареек типа Крона (6LR61)\n'+
                    'Максимальный вес пользователя — 100 кг'
                ,
                category: 'Кровати',
                price: '14 990',
                city:'Krasnodar'
            },
            {
                id: 44,
                title: 'Ortonica Pulse 620',
                img: 'OtornicaPulse620.jpg',
                desc: 'Коляска инвалидная с электроприводом ',
                longDesc:
                    'с обновленным улучшенным мотором мощностью 2х250 Вт.' +
                    ' Новая модификация двигателей обеспечивает плавность и ' +
                    'маневренность хода, а малый вес конструкции и легкое складывание без съема аккумуляторов делают коляску комфортной ' +
                    'для поездок по городу и путешествий на дальние расстояния.\n'+
                    'Технические характеристики:'+
                    'Тип:' +
                    'Кресло-коляска для инвалидов\n' +
                    'Ширина сиденья, см:' +
                    '45,5\n' +
                    'Общая длина, см:' +
                    '113\n' +
                    'Высота спинки, см:' +
                    '38\n' +
                    'Общая высота, см:' +
                    '88,5\n' +
                    'Высота подлокотников, см:' +
                    '24\n' +
                    'Тип аккумулятора:' +
                    'литиевый\n' +
                    'Мощность двигателя:' +
                    '2*250 W\n' +
                    'Емкость аккумулятора:' +
                    '2*12 Ah\n' +
                    'Ремни безопасности:' +
                    'Да\n' +
                    'Подголовник:' +
                    'Нет\n' +
                    'Высота сиденья, см:' +
                    '47,5\n' +
                    'Глубина сиденья, см:' +
                    '44\n' +
                    'Тип привода:' +
                    'электрический\n' +
                    'Общая ширина, см:' +
                    '55\n' +
                    'Количество колес:' +
                    '4\n' +
                    'Тип тормоза:' +
                    'Электромагнитный\n' +
                    'Тип колес:' +
                    'Литые/пневматические\n' +
                    'Механизм складывания:' +
                    'Книжка\n' +
                    'Стояночный тормоз:' +
                    'Нет\n' +
                    'Скорость, км/ч:' +
                    '8\n' +
                    'Максимальная высота препятствия, см:' +
                    '4\n' +
                    'Пробег без подзарядки, км:' +
                    'до 20\n' +
                    'Модель:' +
                    'Pulse 620\n' +
                    'Материал:' +
                    'Алюминий\n' +
                    'Вес, кг (± 5%):' +
                    '28,5\n' +
                    'Грузоподъемность, кг:' +
                    '120\n' +
                    'Угол наклона спинки,°:' +
                    '97\n' +
                    'Длина подножек, см:' +
                    '40-44'
                ,
                category: 'Средства реабилитации',
                price: '124 990',
                city:'Krasnodar'
            },
            {
                id: 45,
                title: 'Ortonica Pulse 620',
                img: 'OtornicaPulse620.jpg',
                desc: 'Коляска инвалидная с электроприводом ',
                longDesc:
                    'с обновленным улучшенным мотором мощностью 2х250 Вт.' +
                    ' Новая модификация двигателей обеспечивает плавность и ' +
                    'маневренность хода, а малый вес конструкции и легкое складывание без съема аккумуляторов делают коляску комфортной ' +
                    'для поездок по городу и путешествий на дальние расстояния.\n'+
                    'Технические характеристики:'+
                    'Тип:' +
                    'Кресло-коляска для инвалидов\n' +
                    'Ширина сиденья, см:' +
                    '45,5\n' +
                    'Общая длина, см:' +
                    '113\n' +
                    'Высота спинки, см:' +
                    '38\n' +
                    'Общая высота, см:' +
                    '88,5\n' +
                    'Высота подлокотников, см:' +
                    '24\n' +
                    'Тип аккумулятора:' +
                    'литиевый\n' +
                    'Мощность двигателя:' +
                    '2*250 W\n' +
                    'Емкость аккумулятора:' +
                    '2*12 Ah\n' +
                    'Ремни безопасности:' +
                    'Да\n' +
                    'Подголовник:' +
                    'Нет\n' +
                    'Высота сиденья, см:' +
                    '47,5\n' +
                    'Глубина сиденья, см:' +
                    '44\n' +
                    'Тип привода:' +
                    'электрический\n' +
                    'Общая ширина, см:' +
                    '55\n' +
                    'Количество колес:' +
                    '4\n' +
                    'Тип тормоза:' +
                    'Электромагнитный\n' +
                    'Тип колес:' +
                    'Литые/пневматические\n' +
                    'Механизм складывания:' +
                    'Книжка\n' +
                    'Стояночный тормоз:' +
                    'Нет\n' +
                    'Скорость, км/ч:' +
                    '8\n' +
                    'Максимальная высота препятствия, см:' +
                    '4\n' +
                    'Пробег без подзарядки, км:' +
                    'до 20\n' +
                    'Модель:' +
                    'Pulse 620\n' +
                    'Материал:' +
                    'Алюминий\n' +
                    'Вес, кг (± 5%):' +
                    '28,5\n' +
                    'Грузоподъемность, кг:' +
                    '120\n' +
                    'Угол наклона спинки,°:' +
                    '97\n' +
                    'Длина подножек, см:' +
                    '40-44'
                ,
                category: 'Средства реабилитации',
                price: '124 990',
                city:'Izhevsk'
            },
            {
                id: 46,
                title: 'Ortonica Pulse 620',
                img: 'OtornicaPulse620.jpg',
                desc: 'Коляска инвалидная с электроприводом ',
                longDesc:
                    'с обновленным улучшенным мотором мощностью 2х250 Вт.' +
                    ' Новая модификация двигателей обеспечивает плавность и ' +
                    'маневренность хода, а малый вес конструкции и легкое складывание без съема аккумуляторов делают коляску комфортной ' +
                    'для поездок по городу и путешествий на дальние расстояния.\n'+
                    'Технические характеристики:'+
                    'Тип:' +
                    'Кресло-коляска для инвалидов\n' +
                    'Ширина сиденья, см:' +
                    '45,5\n' +
                    'Общая длина, см:' +
                    '113\n' +
                    'Высота спинки, см:' +
                    '38\n' +
                    'Общая высота, см:' +
                    '88,5\n' +
                    'Высота подлокотников, см:' +
                    '24\n' +
                    'Тип аккумулятора:' +
                    'литиевый\n' +
                    'Мощность двигателя:' +
                    '2*250 W\n' +
                    'Емкость аккумулятора:' +
                    '2*12 Ah\n' +
                    'Ремни безопасности:' +
                    'Да\n' +
                    'Подголовник:' +
                    'Нет\n' +
                    'Высота сиденья, см:' +
                    '47,5\n' +
                    'Глубина сиденья, см:' +
                    '44\n' +
                    'Тип привода:' +
                    'электрический\n' +
                    'Общая ширина, см:' +
                    '55\n' +
                    'Количество колес:' +
                    '4\n' +
                    'Тип тормоза:' +
                    'Электромагнитный\n' +
                    'Тип колес:' +
                    'Литые/пневматические\n' +
                    'Механизм складывания:' +
                    'Книжка\n' +
                    'Стояночный тормоз:' +
                    'Нет\n' +
                    'Скорость, км/ч:' +
                    '8\n' +
                    'Максимальная высота препятствия, см:' +
                    '4\n' +
                    'Пробег без подзарядки, км:' +
                    'до 20\n' +
                    'Модель:' +
                    'Pulse 620\n' +
                    'Материал:' +
                    'Алюминий\n' +
                    'Вес, кг (± 5%):' +
                    '28,5\n' +
                    'Грузоподъемность, кг:' +
                    '120\n' +
                    'Угол наклона спинки,°:' +
                    '97\n' +
                    'Длина подножек, см:' +
                    '40-44'
                ,
                category: 'Средства реабилитации',
                price: '124 990',
                city:'Ekat'
            },
            {
                id: 47,
                title: 'iWALK3.0',
                img: 'iWalk.jpg',
                desc: 'Инновационный подколенный костыль - свободные руки',
                longDesc: 'Инновационный костыль iWALK отмечен наградами. iWALK - лучшая альтернатива обычным костылям и коленным самокатам при всех травмах ниже колена.\n'+
                    'Костыль iWALK дает вам мобильность без помощи рук / боли и возвращает вам возможность вести функциональный и независимый образ жизни.\n'+
                    'С костылем iWALK снова можно толкать тележку для покупок, подниматься по лестнице, гулять с собакой и ходить на работу.',
                category: 'Средства реабилитации',
                price: '',
                priceA: '2 990 ₽. за месяц',
                city:'Krasnodar'
            },
            {
                id: 48,
                title: 'MET DM-370',
                img: 'MET.jpg',
                desc: 'Медицинская кровать механическая четырехсекционная',
                longDesc: 'Медицинская четырёхсекционная кровать с независимыми червячными регулировками изголовья и ножных секций. Кровать MET DM-370 обладает великолепным качеством исполнения, оснащена складными боковыми ограждениями, быстросъёмными торцевыми спинками из ABS-пластика и всеми остальными конструктивными решениями, эффективно участвующими в выводе вашего стационара на новый уровень как по результативности проводимой терапии, так и по комфорту рабочих условий для персонала.\n'+
                'Кровать - 4-х секционная, снабжена винтовыми механическими приводами регулировки угла наклона спинной секции и угла наклона ножной секции\n' +
                    'Съемные спинки головы и ног, а также угловые бампера-отбойники выполнены из объёмно-формованного ABS-пластика\n' +
                    'Выдвижные регуляторы положения ложа расположены в ножной части кровати, что позволяет персоналу легко регулировать положение ложа\n' +
                    'Боковые ограждения (2шт) выполнены из алюминия и легко опускаются, обеспечивая быстрый доступ к пациенту\n' +
                    'Основание кровати выполнено из стали, покрыто ударопрочной эпоксидной эмалью\n' +
                    'Несущие колеса кровати установлены в основаниях из высокопрочной стали, что обеспечивает их долговечность, а сами колёса из немаркой резины\n' +
                    'Колеса с широкими противопылевыми колпаками также оснащены индивидуальными тормозами-фиксаторами.\n'+
                'Габариты кровати, мм:2100*970\n' +

                    'Габариты ложа, мм: 1950*900"\n' +

                    'Максимальная нагрузка, кг: 250\\n' +

                    'Угол наклона Трен/Антитрен:Нет °\n' +
                    'Высота ложа, мм:500\n' +
                    'Регулировка секций:Механический привод\n' +
                    'Регулировка высоты:Нет\n' +

                    'Функция "кардио-кресло":Частичная\n' +
                    'Функция "переворот:Нет"\n' +

                    'Боковые ограждения:Складные\n' +

                    'Штанга для подтягивания:Да\n' +
                    'Столик:Опция\n' +

                    'Инфузионная стойка:Да\n' +


                    'Количество секций ложа:4\n' +

                    'Спинки:Быстросъёмные, объёмноформованные из ABS-пластика, с удобным ручками по углам и кардхолдером\n' +
                    'Колёса:Самоориентирующиеся, стальные, с резиновым покрытием и широкими пластиковыми корпусами, диаметр 125\n' +
                    'Тормоза:Индивидуальные тормоза-фиксаторы, расположенные не менее, чем на двух колёсах\n',
                category: 'Кровати',
                price: '35900',
                priceA: '3990 р за месяц',
                city:'Izhevsk'
            },
            {
                id: 49,
                title: 'Ходунки Ortonica XS308',
                img: 'xs308.jpg',
                desc: 'Ходунки',
                longDesc: 'предназначены для людей с нарушением опорно-двигательного аппарата. ' +
                    'Ходунки оснащены двумя уровнями опор на разной высоте для облегчения подъема и позволяют передвигаться методом поочередной перестановки левой\n' +
                    'и правой опорных сторон по ходу движения,\n' +
                    'что дает возможность двигаться достаточно быстро, не теряя равновесия.\n' +
                    'Тип: Ходунки\n' +
                    'Цвет рамы: Серебристый матовый\n' +
                    'Общая длина, см:44-45,5\n' +
                    'Общая высота, см:75-93\n' +
                    'Модель:XS308\n' +
                    'Материал:Алюминий\n' +
                    'Вес, кг (± 5%):2,7\n' +
                    'Общая ширина, см:53\n' +
                    'Расстояние между поручнями, см:45\n' +
                    'Грузоподъемность, кг:100\n' +
                    'Длина в сложенном состоянии, см:7,5' ,
                category: 'Средства реабилитации',
                price: '3 990',
                priceA: '590 р. за месяц',
                city:'Izhevsk'
            },
            {
                id: 50,
                title: 'Ходунки Ortonica XS308',
                img: 'xs308.jpg',
                desc: 'Ходунки',
                longDesc: 'предназначены для людей с нарушением опорно-двигательного аппарата. ' +
                    'Ходунки оснащены двумя уровнями опор на разной высоте для облегчения подъема и позволяют передвигаться методом поочередной перестановки левой\n' +
                    'и правой опорных сторон по ходу движения,\n' +
                    'что дает возможность двигаться достаточно быстро, не теряя равновесия.\n' +
                    'Тип: Ходунки\n' +
                    'Цвет рамы: Серебристый матовый\n' +
                    'Общая длина, см:44-45,5\n' +
                    'Общая высота, см:75-93\n' +
                    'Модель:XS308\n' +
                    'Материал:Алюминий\n' +
                    'Вес, кг (± 5%):2,7\n' +
                    'Общая ширина, см:53\n' +
                    'Расстояние между поручнями, см:45\n' +
                    'Грузоподъемность, кг:100\n' +
                    'Длина в сложенном состоянии, см:7,5' ,
                category: 'Средства реабилитации',
                price: '3 990',
                priceA: '590 р. за месяц',
                city:'Ekat'
            },
            {
                id: 51,
                title: 'Ortonica XS308',
                img: 'xs308.jpg',
                desc: 'Ходунки',
                longDesc: 'предназначены для людей с нарушением опорно-двигательного аппарата. ' +
                    'Ходунки оснащены двумя уровнями опор на разной высоте для облегчения подъема и позволяют передвигаться методом поочередной перестановки левой\n' +
                    'и правой опорных сторон по ходу движения,\n' +
                    'что дает возможность двигаться достаточно быстро, не теряя равновесия.\n' +
                    'Тип: Ходунки\n' +
                    'Цвет рамы: Серебристый матовый\n' +
                    'Общая длина, см:44-45,5\n' +
                    'Общая высота, см:75-93\n' +
                    'Модель:XS308\n' +
                    'Материал:Алюминий\n' +
                    'Вес, кг (± 5%):2,7\n' +
                    'Общая ширина, см:53\n' +
                    'Расстояние между поручнями, см:45\n' +
                    'Грузоподъемность, кг:100\n' +
                    'Длина в сложенном состоянии, см:7,5' ,
                category: 'Средства реабилитации',
                price: '3 990',
                priceA: '590 р. за месяц',
                city:'Krasnodar'
            },
            {
                id: 52,
                title: 'Ortonica XS305',
                img: 'xs305.jpg',
                desc: 'Ходунки',
                longDesc: 'предназначены для людей с нарушением опорно-двигательного аппарата. ' +
                    'Замок на передней части конструкции позволяет передвигаться как в фиксированном, так и в шагающем режиме. ' +
                    'Конструкция ходунков складывается и раскладывается за счет нажатия на фиксаторы пальцами, ладонью или ребром кисти. ' +
                    'Ножки подстраиваются под рост пользователя легким нажатием на кнопочный замок, уменьшая нагрузку на суставы\n' +
                    'Тип: Ходунки\n' +
                    'Цвет рамы: Серебристый матовый\n' +
                    'Общая длина, см:45-49\n' +
                    'Общая высота, см:75-93\n' +
                    'Модель:XS305\n' +
                    'Материал:Алюминий\n' +
                    'Вес, кг (± 5%):2,45\n' +
                    'Общая ширина, см:57-59\n' +
                    'Расстояние между поручнями, см:46\n' +
                    'Грузоподъемность, кг:100\n' +
                    'Длина в сложенном состоянии, см:7,5' ,
                category: 'Средства реабилитации',
                price: '',
                priceA: '',
                city:'Krasnodar'
            },
            {
                id: 54,
                title: 'Ortonica XS305',
                img: 'xs305.jpg',
                desc: 'Ходунки',
                longDesc: 'предназначены для людей с нарушением опорно-двигательного аппарата. ' +
                    'Замок на передней части конструкции позволяет передвигаться как в фиксированном, так и в шагающем режиме. ' +
                    'Конструкция ходунков складывается и раскладывается за счет нажатия на фиксаторы пальцами, ладонью или ребром кисти. ' +
                    'Ножки подстраиваются под рост пользователя легким нажатием на кнопочный замок, уменьшая нагрузку на суставы\n' +
                    'Тип: Ходунки\n' +
                    'Цвет рамы: Серебристый матовый\n' +
                    'Общая длина, см:45-49\n' +
                    'Общая высота, см:75-93\n' +
                    'Модель:XS305\n' +
                    'Материал:Алюминий\n' +
                    'Вес, кг (± 5%):2,45\n' +
                    'Общая ширина, см:57-59\n' +
                    'Расстояние между поручнями, см:46\n' +
                    'Грузоподъемность, кг:100\n' +
                    'Длина в сложенном состоянии, см:7,5' ,
                category: 'Средства реабилитации',
                price: '',
                priceA: '',
                city:'Ekat'
            },
            {
                id: 55,
                title: 'Ortonica XR204',
                img: 'xr204.jpg',
                desc: 'Ходунки',
                longDesc: 'Ходунки Ortonica XR204 оснащены передними цельнолитыми колесами на опорных ножках, ' +
                    'не требующими подкачивания. Ножки, регулируемые по высоте, ' +
                    'дают возможность настроить высоту под рост пользователя без применения инструмента, ' +
                    'позволяя минимизировать нагрузки на позвоночник и суставы. Замок на передней части конструкции позволяет передвигаться как в фиксированном, ' +
                    'так и в шагающем режиме. Складная конструкция и легкий, ' +
                    'но прочный алюминиевый каркас делают изделие удобным для хранения и транспортировки.'+
                    'Тип:Ходунки роллаторы\n' +
                    'Цвет:Серебристый\n' +
                    'Диаметр колес (передние), см: 12,5\n' +
                    'Модель:XR204\n' +
                    'Материал:Алюминий\n' +
                    'Вес, кг (± 5%):2,65\n' +
                    'Общая ширина, см:60,5\n' +
                    'Расстояние между поручнями, см:44,5\n' +
                    'Грузоподъемность, кг: 100\n' +
                    'Общая длина, см:52,5-56,5 \n' +
                    'Общая высота, см:78-94 \n' +
                    'Длина в сложенном состоянии, см:13 ',
                category: 'Средства реабилитации',
                price: '4 290',
                priceA: '690 за месяц',
                city:'Izhevsk'
            },
            {
                id: 56,
                title: 'Ortonica XR204',
                img: 'xr204.jpg',
                desc: 'Ходунки',
                longDesc: 'Ходунки Ortonica XR204 оснащены передними цельнолитыми колесами на опорных ножках, ' +
                    'не требующими подкачивания. Ножки, регулируемые по высоте, ' +
                    'дают возможность настроить высоту под рост пользователя без применения инструмента, ' +
                    'позволяя минимизировать нагрузки на позвоночник и суставы. Замок на передней части конструкции позволяет передвигаться как в фиксированном, ' +
                    'так и в шагающем режиме. Складная конструкция и легкий, ' +
                    'но прочный алюминиевый каркас делают изделие удобным для хранения и транспортировки.'+
                    'Тип:Ходунки роллаторы\n' +
                    'Цвет:Серебристый\n' +
                    'Диаметр колес (передние), см: 12,5\n' +
                    'Модель:XR204\n' +
                    'Материал:Алюминий\n' +
                    'Вес, кг (± 5%):2,65\n' +
                    'Общая ширина, см:60,5\n' +
                    'Расстояние между поручнями, см:44,5\n' +
                    'Грузоподъемность, кг: 100\n' +
                    'Общая длина, см:52,5-56,5 \n' +
                    'Общая высота, см:78-94 \n' +
                    'Длина в сложенном состоянии, см:13 ',
                category: 'Средства реабилитации',
                price: '4 290',
                priceA: '690 за месяц',
                city:'Ekat'
            },
            {
                id: 57,
                title: 'Ortonica XS311',
                img: 'xs311.jpg',
                desc: 'Ходунки',
                longDesc: 'идеально подходят для пользователей с временной потерей двигательной активности'+
                    'Тип:Ходунки\n' +
                    'Цвет:Черно-серебристый\n' +
                    'Длина в сложенном состоянии, см:79\n' +
                    'Общая длина, см:79/89\n' +
                    'Ширина в сложенном состоянии, см:42 \n' +
                    'Грузоподъемность, кг:110 кг \n' +
                    'Общая высота, см: 79-103\n' +
                    'Общая ширина, см: 42 \n' +
                    'Диаметр колес (передние/задние), см: 18,5/18,5\n' +
                    'Вес, кг (± 5%):10,2 \n' +
                    'Модель: XS311\n' +
                    'Материал:Сталь \n' ,
                category: 'Средства реабилитации',
                price: '',
                priceA: '2 990 ₽. за месяц',
                city:'Krasnodar'
            },
            {
                id: 57,
                title: 'Ortonica XS311',
                img: 'xs311.jpg',
                desc: 'Ходунки',
                longDesc: 'идеально подходят для пользователей с временной потерей двигательной активности'+
                    'Тип:Ходунки\n' +
                    'Цвет:Черно-серебристый\n' +
                    'Длина в сложенном состоянии, см:79\n' +
                    'Общая длина, см:79/89\n' +
                    'Ширина в сложенном состоянии, см:42 \n' +
                    'Грузоподъемность, кг:110 кг \n' +
                    'Общая высота, см: 79-103\n' +
                    'Общая ширина, см: 42 \n' +
                    'Диаметр колес (передние/задние), см: 18,5/18,5\n' +
                    'Вес, кг (± 5%):10,2 \n' +
                    'Модель: XS311\n' +
                    'Материал:Сталь \n' ,
                category: 'Средства реабилитации',
                price: '',
                priceA: '',
                city:'Izhevsk'
            },
            ,
            {
                id: 58,
                title: 'Костыли подмышечные медицинские',
                img: 'KostilSML.jpg',
                desc: 'Костыли из высокопрочного сплава. Размеры S,M,L',
                longDesc: 'Кoстыли с пoдмышечной опорoй предназначены для дополнительной опoры и облегчения пеpемещения людей имеющих тpавмы и забoлевaния нижниx конечностей.\n'+

                    'Веc пaры костылей – 2,35 кг.\n'+
                    'Выдepживaют нагрузкy до 150 кг.\n'+
                    'Диапазoн рeгyлиpoвки выcoты костыля:\n'+
                    'Высота коcтыля oт пoла до пoдмышечника: MIN 1070, MAX 1315 mm. \n' ,
                category: 'Средства реабилитации',
                price: '2 490',
                priceA: '490 р за месяц',
                city:'Izhevsk'
            }            ,
            {
                id: 60,
                title: 'Носилки медицинские',
                img: 'Nosilki.png',
                desc: 'Носилки медицинские поперечно продольно складные',
                longDesc: 'Регистрационное удостоверение Росздравнадзора РЗН 2021/14332 от 14.05.2021\n' +
                    'Носилки имеют жёсткие опоры, брусья из алюминиевого сплава, ложе из полимерной ткани.\n' +
                    'Носилки медицинские складные предназначены для транспортировки пациентов в зонах действия экстренной медицинской помощи и служб спасения при проведении аварийно-спасательных работ.\n' +
                    'При хранении и транспортировке, в сложенном состоянии, имеют маленькие размеры.\n' +
                    'Особенности:\n' +
                    '• Складывание носилок в продольном и поперечном направлениях обеспечивает компактность и удобство;\n' +
                    '• В сложенном виде убираются в чехол;\n' +
                    '• Четыре опоры для размещения на твёрдой поверхности;\n' +
                    '• Брусья из прочного алюминиевого сплава обеспечивают носилкам небольшой вес и достаточную жёсткость при транспортировке пациента;\n' +
                    '• Ложе из специальной полимерной ткани: легко дезинфицируется, облегчает вес, удобно для транспортировки больного и более практично, по сравнению с брезентом.\n' +
                    'Технические характеристики:\n' +
                    '• Длина носилок (полотно без ручек)- 173 см;\n' +
                    '• Длина носилок (с ручками) - 207 см;\n' +
                    '• Ширина носилок - 55 см;\n' +
                    '• Высота носилок (от поверхности) - 11 см;\n' +
                    '• Максимальная нагрузка - 160 кг;\n' +
                    '• Цвет носилок - Оранжевый;\n' +
                    '• Регулируемые ремни безопасности (длина 180 см) - 2 шт.;\n' +
                    '• Вес нетто - 4,6 кг.\n' +
                    '• Вес брутто - 5,2 кг\n' +
                    '• Габариты в чехле - 105х10х20 см;\n' +
                    '• Габариты упаковки - 107х11х20 см' ,
                category: 'Средства реабилитации',
                price: '7 990',
                priceA: '790 р за 3 дня',
                city:'Izhevsk'
            } ,
            {
                id: 61,
                title: 'Трость опорная',
                img: 'Opor_Trost.png',
                desc: 'для ходьбы телескопическая новая',
                longDesc: 'Отeчeствeннoго пpоизвoдcтвa\n' +
                    '✅Троcть имeющaя вoзмoжнocть регулировки пo высoте\n' +
                    '✅Этa троcть oтечественногo пpоизводства идeaльнo пoдoйдeт pазличным категориям пользователей. ✅Благодаря своей универсальности, она предоставляет надежную опору и комфорт во время передвижения.\n' +
                    '✅Кроме того, данная модель оснащена накладным устройством противоскольжения, что делает ее идеальной для использования в зимнее время года.\n' +
                    'Основные характеристики:\n' +
                    '▪️Оснащение: Накладное УПС\n' +
                    '▪️Количество секций: 2\n' +
                    '▪️Материал трости: Алюминиевый сплав\n' +
                    '▪️Устойчивость к коррозии: да\n' +
                    '▪️Высота ( ± 5%): 720-945 мм\n' +
                    '▪️Грузоподъемность: 110\n' ,
                category: 'Средства реабилитации',
                price: '790',
                city:'Izhevsk'
            },
            {
                id: 62,
                title: 'Кислородный концентратор Longfian J10',
                img: 'J10.jpg',
                desc: 'Производительность 10 литров в минуту',
                longDesc: 'Самый мощный из кислородных концентраторов\n' +
                    ' \n' +
                    'Если сомневайтесь какой из аппаратов приобрести ? Тогда выбирайте кислородный концентратор Longfian JAY -10!\n' +
                    'Вы всегда можете выбрать подходящий для вас литраж . При этом концентрация кислорода не будет снижаться как в других моделях\n' +
                    '\n' +
                    'Стабильная подача кислорода на всех режимах работы: \n' +
                    '1 литр/минуту концентрация :93±3% \n' +
                    '2 литра/минуту концентрация: 93±3% \n' +
                    '3 литра/минуту концентрация: 93±3% \n' +
                    '4 литра/ минуту концентрация: 93±3% \n' +
                    '5 литров/ минуту концентрация: 93±3% \n' +
                    '6 литров/ минуту концентрация: 93±3% \n' +
                    '7 литров/ минуту концентрация: 93±3% \n' +
                    '8 литров/ минуту концентрация: 93±3% \n' +
                    '9 литров/ минуту концентрация: 93±3% \n' +
                    '10 литров/ минуту концентрация: 93±3% \n' +
                    '\n' +
                    '✅Концентратор кислорода JAY 10 с двумя выходами кислорода – позволяет одновременно проводить оксигенотерапию двум пациентам. \n' +
                    '✅Для изменения скорости воздушного потока предусмотрены 2 независимых механических регулятора.\n' +
                    '✅Это экономичный и эффективный высокопроизводительный концентратор, предназначенный для использования в больницах, медицинских центрах, реанимационных отделениях и отделениях интенсивной терапии а также для домашнего использования \n' +
                    '✅Без проблем выдерживает интенсивную ежедневную нагрузку и рассчитан на срок службы не менее 10 лет. \n' +
                    '\n' +
                    '✔️Максимальный поток – 10 л/мин.\n' +
                    '✔️Удобный отсек для комплектующих.\n' +
                    '✔️Большой ресурс работы.\n' +
                    '✔️ЖК-дисплей отображает общее время работы, время работы текущего цикла и установленное время.\n' +
                    '✔️Устройство комплектуется небулайзером для распыления лекарственных препаратов со скоростью потока от 0,1 мл/мин.\n' +
                    '✔️Надёжная система контроля отслеживает корректность работы прибора\n' +
                    '✔️Концентратор кислорода снабжен колесными опорами для удобства перемещения;\n' +
                    '✔️Расходомер концентратора кислорода имеет шкалу до 10 литров\n' +
                    '✔️Низкий уровень шума при работе концентратора Jay 10 позволяет использовать его в течение ночи\n' +
                    '✔️Прибор снабжён встроенными фильтрами, которые очищают входящий воздух от загрязняющих частиц, аллергенов и вредных примесей.\n' +
                    '\n' +
                    'ХАРАКТЕРИСТИКИ:\n' +
                    '▪️Производительность (л/мин): 0-10 \n' +
                    'Максимальный поток (л/мин): 10 \n' +
                    'Концентрация кислорода на выходе: 93±3%\n' +
                    '▪️Давление на выходе (МПа): 0,04-0,07 ▪️Рабочее напряжение (В/Гц): 220±10% /50±1\n' +
                    '▪️Уровень шума (дБ): <55 \n' +
                    '▪️Мощность (ВА): 600 \n' +
                    '▪️Вес нетто (кг): 27 \n' +
                    '▪️Вес брутто (кг): 29 \n' +
                    '▪️Размер (ДхШхВ),(мм): 365x375x600 ▪️Габариты в упаковке (ДхШхВ), \n' +
                    '    (мм): 640х435х435' ,
                category: 'Кислородные концентраторы',
                price: '74 000',
                city:'Izhevsk'
            },
            {
                id: 80,
                title: 'Кислородный концентратор Армед 8F-5',
                img: 'Armed8f-5.jpeg',
                desc: 'Производительность 5 литров в минуту',
                longDesc: 'Полноценный пятилитровый концентратор, находящий широкое применение как в домашних, так и в стационарных условиях. Одно из преимуществ - возможность использования концентратора как основного или резервного источника кислорода.\n'+
                '• Может работать 24 часа в сутки от обычной розетки 220В\n' +
                    '• Плавная регулировка объема воздушно-кислородного потока\n' +
                    '• Встроен счетчик наработанного времени\n' +
                    '• Аварийная сигнализация (зуммер) при экстренном отключении электричества\n' +
                    '• Имеется индикатор электропитания и индикатор неисправности',
                category: 'Кислородные концентраторы',
                price: '42 990',
                city:'Izhevsk'
            },
            {
                id: 85,
                title: 'Кислородный концентратор Longfian JAY-5',
                img: 'J5_white.jpeg',
                desc: 'Производительность 5 литров в минуту',
                longDesc: 'Кислородный концентратор  Longfian Jay-5 производительностью до 5литров/мин. с информативным ЖК-дисплеем.\n' +
                    'Компактный, лёгкий концентратор с уровнем шума не превышающим 40Дб.\n' +
                    'Подготовка кислорода происходит по технологии адсорбции короткого цикла (PSA). Компрессором подается профильтрованный воздух на колонки, производящие кислород. Они содержат порошковый адсорбент, который отделяет кислород от азота и других веществ, содержащихся в окружающем воздухе. Затем, вырабатываемый прибором кислород проходит через бактериальный фильтр и подается непосредственно к пациенту.\n' +
                    'Благодаря своим компактным размерам, небольшому уровню шума и низкому энергопотреблению кислородный концентратор Longfian Jay-5 отлично подходит для эксплуатации как в лечебных учреждениях, так и в домашних условиях.\n'+
                    'Производительность: 0.5-5 л/мин\n' +
                    'Концентрация кислорода: 93 ± 3%\n' +
                    'Уровень шума:≤ 40 Дб\n' +
                    'Мощность:350 Вт\n' +
                    'Таймер:есть\n' +
                    'Питание:220В, 50Гц\n' +
                    'Габаритные размеры:35х54х28 см\n' +
                    'Вес [кг]:16 кг\n',
                category: 'Кислородные концентраторы',
                price: '42 990',
                city:'Izhevsk'
            },
            {
                id: 90,
                title: 'Кислородный концентратор Армед 7F-5C',
                img: 'Armed7f-5c.jpeg',
                desc: 'Производительность 5 литров в минуту',
                longDesc: 'Кислородный концентратор Армед 7F-5C производительностью 5 л/мин ' +
                    'применяется для профилактики и лечения заболеваний, вызванных дефицитом кислорода. ' +
                    'Концентратор кислорода используется как в домашних условиях, так и в медучреждениях. ' +
                    'Панель управления проста и удобна. На дисплее отображается время работы в минутах, а индикатор оповещает о рабочем состоянии техники. ' +
                    'В конструкции нет кислородных баллонов, что делает аппарат безопасным в использовании. ' +
                    'Благодаря колесам и небольшим размерам концентратор легко перемещать. Канюля в комплекте.'+
                'Концентрация КВС на выходе при максимальной производительности:' +
                    '≥ 93 %\n' +
                    'Уровень шума (не более):' +
                    '55 дБА\n' +
                    'Вес нетто (ед):' +
                    '16.9 кг\n' +
                    'Время выхода концентратора на рабочий режим:' +
                    '3 мин\n' +
                    'Вес брутто (ед):' +
                    '19 кг\n' +
                    'Упаковка (ед):' +
                    'Картонная коробка\n' +
                    'Габариты в упаковке (ед):' +
                    '49.5*35.5*58.5 см',
                category: 'Кислородные концентраторы',
                price: '39 990',
                city:'Izhevsk'
            },
            {
                id: 91,
                title: 'СИПАП аппарат Аксион АДТ-01',
                img: 'AxionSipap.jpg',
                desc: 'Аппарат дыхательной терапии апноэ',
                longDesc: 'Профессиональный аппарат дыхательной терапии АДТ-01 "Аксион" - ваш надежный помощник!\n' +
                    '\n' +
                    'Производитель Россия 🇷🇺 \n' +
                    '\n' +
                    'Маска в подарок \n' +
                    ' \n' +
                    'АДТ-01 - это специализированный СИПАП аппарат, разработанный для эффективной поддержки дыхания пациентов с синдромом апноэ. Этот медицинский термин обозначает остановку дыхания во время сна более чем на 10 секунд.\n' +
                    '\n' +
                    '\n' +
                    ' Преимущества использования АДТ-01:\n' +
                    '- Подача достаточного количества воздуха при затрудненном или недостаточном дыхании.\n' +
                    '- Поддержание самостоятельного дыхания пациентов массой не менее 40 кг.\n' +
                    '- Подходит для применения как в медицинских учреждениях, так и дома под контролем врача.\n' +
                    '\n' +
                    'Особенности :\n' +
                    '-Компактный дизайн с удобным интерфейсом\n' +
                    '-Герметичный интеллектуальный увлажнитель — непрерывный мониторинг уровня воды обеспечивает максимальную безопасность\n' +
                    '-Многочисленные терапевтические параметры для лучшей клинической оценки\n' +
                    '-Высокоинформативный цветной дисплей\n' +
                    '-Уникальный режим IPR (интеллектуальный сброс давления) алгоритм обеспечивает больший  комфорт во время вентиляционной терапии\n' +
                    '-Режим CPAP постоянное положительное давление в дыхательных путях с настраиваемым давлением от 4 до 20 см. вод. ст.\n' +
                    '-Режим E-COMP (функция комфортной адаптации) позволяет начать процедуры с 50% давлением, которое затем  будет увеличиваться с шагом 1 см. вод. ст.\n' +
                    '-Режим APAP постоянное положительное давление в выходных потоках с настраиваемым и автоматическим изменением давления от 4 до 20 см. вод. ст.\n' +
                    '\n' +
                    'Характеристики:\n' +
                    '-Режимы работы: CPAP, APAP , CPAP с функцией E-COMP\n' +
                    '-Слот для SD-карты для записи показателей\n' +
                    '-Звуковые сигналы оповещения\n' +
                    '-Степень защиты от проникновения воды и пыли IP22\n' +
                    '-Средняя наработка на отказ не менее 2000 часов\n' +
                    '\n' +
                    'Комплектация:\n' +
                    '▪️блок управления\n' +
                    '▪️коннектор дыхательного шланга\n' +
                    '▪️дыхательный контур\n' +
                    '▪️маска\n' +
                    '▪️блок питания 220/24\n' +
                    '\n' +
                    'Принадлежности:\n' +
                    '▪️карта SD\n' +
                    '▪️фильтр воздушный 2 шт\n' +
                    '▪️сумка для переноски',
                category: 'Приборы',
                price: '52 199',
                city:'Izhevsk'
            },
            {
                id: 92,
                title: 'СИПАП-аппарат Resvent Ibreeze 20A',
                img: 'SipUpResvent.jpg',
                desc: 'Аппарат дыхательной терапии апноэ',
                longDesc: 'Автоматический CPAP ResVent iBreeze 20A  позволяет проводить качественную СИПАП-терапию с целью устранения остановок дыхания во сне. Анализируя физическое состояние пациентов в реальном времени с помощью алгоритма REG, прибор автоматически поддерживает оптимальное давление .\n' +
                    'СИПАП ResVent 20A – самый компактный и легкий стационарный аппарат\n' +
                    '\n' +
                    'Аппарат автоматический – позволяет устранить ночные эпизоды апноэ и храп.\n' +
                    'Устранение апноэ осуществляется с помощью подачи положительного или отрицательного давления в дыхательные пути, в результате этого дыхательный просвет расширяется. Устранение апноэ и связанной с ним гипоксии уже на следующий день отражается в виде улучшения самочувствия, полноценного сна в глубокой стадии. В результате чего пациент бодрый в течении всего дня, со временем гормональный фон и обмен веществ нормализуется, что способствует похудению и более активному образу жизни.\n' +
                    '\n' +
                    'Маска в подарок \n' +
                    '\n' +
                    'Особенности и преимущества iBreeze 20A\n' +
                    ' • Лечебное давление от 3-20 смH20, автоматический режим работы\n' +
                    ' • Режим autoRamp помогает пациенту заснуть. Аппарат анализирует поведение пациента, поддерживает максимальный комфорт и низкое давление до момента, когда наступит сон. Затем аппарат перейдет в стандартный режим работы, основываясь на предустановленных настройках.\n' +
                    ' • Функция E-COMP помогает новым пациентам адаптироваться к целевому давлению с шагом 1 см H2O в день, начиная с 50% от необходимого лечебного давления, прописанного в результате диагностики\n' +
                    ' • Пользовательский интерфейс интуитивно понятен, имеется яркий цветной дисплей, русифицированное меню\n' +
                    ' • Интеллектуальный увлажнитель iBreeze 20A обеспечивает непрерывный контроль уровня воды и максимальную безопасность. Автоматически отключается, при отсутствии воды.\n' +
                    ' • Уникальная интеллектуальная функция сброса давления (IPR) позволяет легче дышать во время сна, обеспечивая больший комфорт.\n' +
                    ' • Вес аппарата всего 1,35 кг, имеет самые компактные размеры из всех стационарных автоматических СИПАП-аппаратов.          \n' +
                    '\n' +
                    'Характеристики:\n' +
                    'Уровень шума- 28 дБ\n' +
                    'Режимы вентиляции-AutoCPAP, CPAP\n' +
                    'Давление водяного столба (мин/макс) от 4 до 20 см H2O\n' +
                    'Подогрев увлажнителя -да\n' +
                    'Отсрочка сна (RAMP)-от 4 до 45 мин.\n' +
                    'Габариты аппарата без увлажнителя (ГхШхВ)- 238 x 178 x 128 мм\n' +
                    'Вес аппарата без увлажнителя-1,35 кг\n' +
                    '\n' +
                    'Комплектация аппарата:\n' +
                    '-CPAP-аппарат RESVENT 20A-1 шт\n' +
                    '-Увлажнитель  - 1 шт\n' +
                    '-Маска -1 шт\n' +
                    '-Инструкция по эксплуатации - 1 шт\n' +
                    '-Дыхательный контур - 1 шт\n' +
                    '-Сумка для переноски -1шт\n' +
                    '-Сменный фильтр -2 шт',
                category: 'Приборы',
                price: '54 990',
                city:'Izhevsk'
            }
        ]
    }

    const [itemModal, setItemModal] = useState([data.items]);
    const [isAnimating, setIsAnimating] = useState(false);

    function sortByCategory(a, b) {
        return a.category.localeCompare(b.category);
    }


    const filteredData = data.items.filter((item) => {
        // Filtering based on search term
        if (
            searchTerm &&
            !(
                item.title.toLowerCase().includes(searchTerm.toLowerCase())
                || item.desc.toLowerCase().includes(searchTerm.toLowerCase())
            )
        ) {
            return false;
        }

        // Filtering based on selected category
        if
        (selectedCategory === 'all')
        {

        }
        else

        if (
            selectedCategory &&
            !item.category.includes(selectedCategory)

        ) {
            return false;
        }

        // Filtering based on selected city
        if (selectedCity && item.city.toLowerCase() !== selectedCity.toLowerCase()) {
            return false;
        }

        return true;
    }).sort(sortByCategory)

    useEffect(() => {
        setIsAnimating(true);
        const timer = setTimeout(() => setIsAnimating(false), 200); // Задержка для завершения анимации (500 миллисекунд)
        return () => clearTimeout(timer);
    }, [filteredData.length]);

    return (
        <div className={"flex flex-col px-4 py-6 mx-auto bg-gray-50 text-black " + (isModalOpen ? "fixed" :"") } >
            <Head>
                <title>
                    Наши Товары - Аренда и продажа медицинского оборудования
                </title>
                <meta name="description" content="Просмотрите наш ассортимент медицинского оборудования, доступного для аренды и продажи в Ижевске. Выберите то, что подходит именно вам!" />
            </Head>


            <h1 className="text-center  text-3xl font-bold mb-4">Товарный ассортимент</h1>

            <p className={ isAnimating ? "transition ease-out opacity-25  duration-200  " : ""}>    Найдено товаров:   {filteredData.length} </p>
            {/* Search input */}
            <div className="mb-1">
                <label htmlFor="category" >
                    <strong>Название товара:</strong><br></br>
                </label>
                <input
                    type="text"
                    placeholder="Поиск по названию товара"
                    className="w-full rounded-lg border-gray-500 border px-2 py-1"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                />
            </div>

            {/* Category dropdown */}
            <div className="mb-1">
                <label htmlFor="category">
                    <strong>Категория товара:</strong><br></br>
                </label>
                <select
                    id="category"
                    className="w-full rounded-lg border-gray-500 border px-2 py-1"
                    value={selectedCategory}
                    onChange={(e) => setSelectedCategory(e.target.value)}
                >
                    <option value="all">Все товары</option>
                    <option value="Кислородные концентраторы">Кислородные концентраторы</option>
                    <option value="Средства реабилитации">Средства реабилитации</option>
                    <option value="Комплектующие">Комплектующие</option>
                    <option value="Приборы">Приборы</option>
                    <option value="Кровати">Кровати</option>
                </select>
            </div>

            {/* City dropdown */}
            <div className="mb-1">
                <label htmlFor="city" className="mr-2">
                    <strong>Город:</strong>
                </label>
                <select
                    id="city"
                    className="w-full rounded-lg border-gray-500 border px-2 py-1"
                    value={selectedCity}
                    onChange={(e) => setSelectedCity(e.target.value)}
                >

                    <option id="Izhevsk" value="Izhevsk">Ижевск</option>
                    {/*     <option id="Ekat" value="Ekat">Екатеренбург</option> */}
                    <option id="Krasnodar" value="Krasnodar">Краснодар</option>
                </select>
            </div>

            {/* Displaying filtered data */}
            {filteredData.length > 0 ? (

                <ul className="mx-auto  grid sm:grid-cols-2 md:grid-cols-3 grid-cols-1 gap-3 ">

                    {filteredData.map((item) => (
                        <li
                            key={item.id}
                            className="border-gray-300 border-2 mb-4 pb-4 last:mb-0 last:pb-0 text-center justify-center items-center"
                        >
                            <Image className='w-auto h-auto mx-auto justify-center items-center transition-all
                                duration-200 hover:scale-105 '
                                   src={"/ProductImg/" + item.img}
                                   height={150}
                                   width={150}

                                   alt="Товар"/>
                            <h2 className="text-xl font-bold mb-2">{item.title}</h2>
                            <p className="mb-1">{item.desc}</p>
                            {item.price &&
                                <p className="mb-0 font-bold text-blue-700">{`Цена продажи ${item.price} ₽`}</p>
                            }
                            {item.priceA &&
                                <p className="mb-0 font-bold text-blue-800">{`Цена аренды ${item.priceA}`}</p>
                            }
                            <button className="bg-blue-500 hover:bg-blue-800 text-white py-2 px-4 rounded-lg font-bold" onClick={ () => handleOpenModal(item) }>
                                Открыть
                            </button>
                        </li>
                    ))}
                </ul>
            ) : (
                <p className="text-lg text-center"><strong>Товары по выбранным параметрам поиска не найдены</strong></p>
            )}


                <Modal isOpen={isModalOpen} handleClose={handleCloseModal} itemModal = {itemModal}>

                </Modal>




        </div>
    );
};

export default Search;

